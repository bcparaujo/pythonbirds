    ...
    def colidir(self, outro_ator, intervalo=1):
        ...
        if self.status == ATIVO  and outro_ator.status == ATIVO:
            ...
            if delta_x <= intervalo and delta_y <= intervalo:
                self.status = outro_ator.status = DESTRUIDO #Atibuido valor para os dois atribulos, ator e outro ator
...
class Passaro(Ator):
    ...
    def foi_lancado(self):
        ...
        :return: booleano
        """
        return not self._tempo_de_lancamento is None #Se for verdadeira a expressão retorna True, se não, retorna False.
